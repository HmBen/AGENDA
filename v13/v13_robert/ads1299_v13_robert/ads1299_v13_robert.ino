...
void setup() {
  ...
  ADS.WREG(0x01, 0x90 | FS_1k); // CONFIG1, 1000Hz sampling rate
  ADS2.WREG(0x01, 0x90 | FS_1k); // CONFIG1, 1000Hz sampling rate
...
void loop() {
  ...
  if (ADS1299_data_ready) {
    ...
    if (counter == REC_SEC*1000 || logData(buffer, 2*N_CH+4)) {
      // Should end
      rb.sync();
      file.truncate();
      file.close();
